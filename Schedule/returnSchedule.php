...
//                $_SESSION["return_date"] = date('Y-m-d', strtotime($_POST["return_date"]));
                $_SESSION["departureScheduleID"] = $_POST["flightScheduleID"];
                $departflightid=$_SESSION["departureScheduleID"];
                $_SESSION["departureScheduleID"] = $_POST["flightScheduleID"];
                $_SESSION["departure_time"] = $_POST["departure_time"];
                 $_SESSION["departure_arrival_time"] = $_POST["arrival_time"];
                 $_SESSION["departure_time_taken_hour"]= $_POST["time_taken_hour"];
                 $_SESSION["departure_time_taken_min"]= $_POST["time_taken_min"];
                 $_SESSION["departure_flight_name"]= $_POST["flight_name"];
                 $_SESSION["departure_airplane_id"]= $_POST["airplane_id"];
                echo "<script>localStorage.setItem('departureFlightScheduleId', '$departflightid');</script>";
...
//        $GLOBALS['guests'] = $guests;
        ...
        $selected_date = isset($_GET['date']) ? $_GET['date'] : $return_date;
       ...
       <?php    
        // Connect to the database
          ...
          $sql = "SELECT fs.flight_schedule_id, fs.route_id, fs.airplane_id, fs.departure_time, fs.arrival_time, fs.departure_day, fs.price, r.origin, r.destination, r.time_taken_hour, r.time_taken_min, ap.name 
                ...
                WHERE fs.flight_schedule_id = '$departureScheduleID' 
      ...
      if ($result->num_rows > 0) {
          // output data of each row
           while($row = $result->fetch_assoc()) {
                echo'<form action="departureSchedule.php">';
                ...
                echo '<span class="luggage">7kg</span>';
                ...
                echo'<button type = "submit" class="submit-btn">Edit</button>';
                echo '</div>';
                echo '</fieldset>';
                echo'</form>';
           }
        ...
        if(isset($_GET['clicked_date'])){
            $sql = "SELECT fs.flight_schedule_id, fs.route_id, fs.airplane_id, fs.departure_time, fs.arrival_time, fs.departure_day, fs.price, r.origin, r.destination, r.time_taken_hour, r.time_taken_min, ap.name 
                ...
                WHERE fs.departure_day = DAYNAME('$selected_date')
                  AND r.origin = '$destination' 
                  AND r.destination = '$origin' 
         ...
         $sql = "SELECT fs.flight_schedule_id, fs.route_id, fs.airplane_id, fs.departure_time, fs.arrival_time, fs.departure_day, fs.price, r.origin, r.destination, r.time_taken_hour, r.time_taken_min, ap.name 
                ...
                JOIN airplane ap ON fs.airplane_id = ap.airplane_id 
                JOIN seat_detail sd ON ap.airplane_id = sd.airplane_id 
                WHERE fs.departure_day = DAYNAME('$return_date')
                  AND r.origin = '$destination' 
                  AND r.destination = '$origin' 
                   AND starting_date >= '2000-01-01'
                GROUP BY fs.flight_schedule_id;";  
            ...
            $result = $conn->query($sql);
       echo'<h2 class="return" style="margin:20px 20px 20px 700px;">Return</h2>';
      if ($result->num_rows > 0) {
          // output data of each row
           while($row = $result->fetch_assoc()) {
               //if round trip> return schedule, else > add on
                ...
                echo '</div>';
                echo '</fieldset>';
                  $_SESSION["returnScheduleID"] =$row["flight_schedule_id"] ;
                  $_SESSION["returnairplaneID"] =$row["airplane_id"] ;
